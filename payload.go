...
type SeatRange struct {
	...
	_io *kaitai.Stream
	_root *Payload
	_parent *SeatReservationBlock
}
func NewSeatRange() *SeatRange {
	return &SeatRange{
	}
}

func (this SeatRange) IO_() *kaitai.Stream {
	return this._io
}

func (this *SeatRange) Read(io *kaitai.Stream, parent *SeatReservationBlock, root *Payload) (err error) {
	this._io = io
	this._parent = parent
	this._root = root
	...
	this.CarNumber = string(tmp42)
	tmp43, err := this._io.ReadU2be()
	if err != nil {
		return err
	}
	this.SeatNumber = uint16(tmp43)
	tmp44, err := this._io.ReadU2be()
	if err != nil {
		return err
	}
	this.SeatNumber2 = uint16(tmp44)
	return err
}

func (this *SeatRange) String() string {
	if this.SeatNumber == this.SeatNumber2 {
		return fmt.Sprintf("%d", this.SeatNumber)
	} else {
		return fmt.Sprintf("%d-%d", this.SeatNumber, this.SeatNumber2)
	}
}
...
func (this *SeatReservationBlock) CarNumbers() string {
	...
	s := this.SeatRanges[0].CarNumber
	tmp[s] = true
	for i := 1; i < len(this.SeatRanges); i++ {
		s2 := this.SeatRanges[i].CarNumber
		_, found := tmp[s2]
		if ! found {
			tmp[s2] = true
			s += ","
